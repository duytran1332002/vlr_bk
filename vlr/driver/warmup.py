...
class WarmupPolynomialScheduler(_LRScheduler):
    def __init__(self, optimizer, warmup_epochs, num_epochs, iter_per_epoch, power):
        self.base_lrs = {
            param_group["name"]: param_group["lr"]
            for param_group in optimizer.param_groups
        }
    ...
    def get_lr(self, base_lr):
        if self.iter < self.warmup_iter:
            ...
            return (
                base_lr
                * (1 - (self.iter - self.warmup_iter) / decay_iter)**self.power
            )
...
class WarmupStepScheduler(_LRScheduler):
    def __init__(self, optimizer, warmup_epochs, num_epochs, iter_per_epoch, gamma, step_size):
        self.base_lrs = {
            param_group["name"]: param_group["lr"]
            for param_group in optimizer.param_groups
        }
    ...
    def get_lr(self, base_lr):
        if self.iter < self.warmup_iter:
            ...
            return (
                base_lr
                * self.gamma**(np.floor((self.iter - self.warmup_iter) / self.step_size))
            )
