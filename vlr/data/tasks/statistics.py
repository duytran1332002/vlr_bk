...
class Args:
    ...
    final_stage_dir = "/mnt/d/Projects/sandboxes/vlr/v1/stage_4"   # Change this.
    # Path to directory containing transcripts.
    transcript_dir = "/mnt/d/Projects/sandboxes/vlr/v1/transcripts"    # Change this.
    # Path to directory containing statistics.
    statistics_dir = "/mnt/d/Projects/sandboxes/vlr/v1/statistics"   # Change this.
...
def main(args: Args):
    ...
    for channel_name in tqdm(
        ...
        prev_stage_dir = os.path.join(args.final_stage_dir, channel_name + ".json")
        if not os.path.exists(prev_stage_dir):
            print(f"Channel {channel_name} does not exist.")
            continue
        dataset = load_dataset("json", data_files=prev_stage_dir, split="train")
        ...
        for sample in tqdm(
            ...
            transcript_path = os.path.join(args.transcript_dir, channel_name, sample["id"] + ".txt")
