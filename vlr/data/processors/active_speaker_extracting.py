...
from vlr.data.utils.TalkNet.talkNet import talkNet
...
class ActiveSpeakerExtracting(Processor):
    def __init__(self,
                 ...
                 pretrainModel: str = "pretrain_TalkSet.model",
    ...
    def inference_video(self, flist: list) -> list:
        ...
        for fidx, fname in enumerate(flist):
            ...
            bboxes = self.DET.detect_faces(
                imageNumpy, conf_th=0.9, scales=[self.facedetScale])
            dets.append([])
            for bbox in bboxes:
    ...
    def extract_active_speaker(self, flist: list, tracks: tuple, scores: list, videoName : str) -> None:
        # CPU: visulize the result for video format
        for tidx, track in enumerate(tracks):
    ...
    def process(self, videoPath : str) -> None:
        try:
            videoName = os.path.splitext(os.path.basename(videoPath))[0]
            savePath = os.path.join(self.outputPath, videoName)
...
if __name__ == "__main__":
    videoFolderPath = '/home/duytran/Desktop/TalkNet-ASD/demo'
    outputPath = '/home/duytran/Desktop/TalkNet-ASD/test'
    activeSpeakerExtract = ActiveSpeakerExtracting(videoFolderPath=videoFolderPath,
    ...
    multi_pool.map(activeSpeakerExtract.process_video, file_list)
    multi_pool.close()
    multi_pool.join()
